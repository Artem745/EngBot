...
async def quiz1(message: Message, state: FSMContext):
    ...
    await message.answer(
        quiz_quest, reply_markup=builderr.as_markup(resize_keyboard=True)
    )
...
async def quiz(message: Message, state: FSMContext):
    ...
    if msg == "❌":
        ...
        if "win_streak_q" not in user_data:
        ...
        if msg == quiz_answer:
            ...
            if user_data["win_streak_q"] + 1 == 5:
                ...
                await state.update_data(win_streak_q=user_data["win_streak_q"] + 1)
        ...
        await message.answer(
            quiz_quest_new, reply_markup=builderr.as_markup(resize_keyboard=True)
        )
