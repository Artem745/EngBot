...
async def DB_add(user_id, username, first_name, last_name, score=0, tr_flag="0"):
    async with aiosqlite.connect("data/EngBotDb.db") as db:
        ...
        if not await cursor.fetchone():
            await db.execute(
                "INSERT INTO users (user_id, username, first_name, last_name, score, tr_flag) VALUES(?,?,?,?,?,?)",
...
async def DB_score(user_id, score):
    async with aiosqlite.connect("data/EngBotDb.db") as db:
        ...
        if new_score < 0:
            ...
            await db.execute(
                "UPDATE users SET score = ? WHERE user_id = ?", (new_score, user_id)
            )
        await db.commit()
...
async def DB_leaderboard():
    async with aiosqlite.connect("data/EngBotDb.db") as db:
        cursor = await db.execute(
            "SELECT username, first_name, last_name, score FROM users ORDER BY score DESC LIMIT 10"
        )
        ...
        for index, user in enumerate(board):
            if not user[0] and not user[2]:
                ...
                result += f"#<b>{index+1}</b> {user[1]} {user[2]} @{user[0]}. Score: {user[3]}"
            if len(smiles) > index:
...
async def DB_insert(flag, number, user_id):
    async with aiosqlite.connect("data/EngBotDb.db") as db:
        await db.execute(
            f"UPDATE users SET {flag} = ? WHERE user_id = ?", (number, user_id)
        )
        await db.commit()
